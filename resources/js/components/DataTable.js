...
export default function DataTable({tableAttributes, query, limit, offset, updateTotal = f => f})
{
  ...
  const apiurl = _apiurl + '?' + (query ? `query=${query}`:'') + (limit ? `&limit=${limit}`:'') + (offset ? `&start=${offset}`:'');

  const delayTask = useAsyncTaskDelay(useCallback(() => 200, [query]));
  ...
  return (
    <table className={'table' + (tableAttributes.striped===true ? ' table-striped':'')
      + (tableAttributes.dark===true ? ' table-dark':'')
      + (tableAttributes.bordered===true ? ' table-bordered':'')}>
        <DataTableHeader
          columnData={tableAttributes.columnData}
          darkHeader={tableAttributes.darkHeader}
          onSortSelected={ (id) => {
        ...
        />
      <tbody>
      {fetchTask.result.data.map((r,i) => (
        <DataRow
          // key = { i }
          key = {eval('r.' + tableAttributes.primaryKey)}
          { ...r }
        />))}
      </tbody>
    </table>
